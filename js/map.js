...
var drawTrajetButton = document.getElementsByClassName('drawTrajet');
...
$('.btn_finish').click(function () { //faire disparaitre le bouton C finish
...
function addLatLngToBati(clickEventData) {
	var iconeBatiment = $('.selected2').attr('name');
...
drawPolygonButton.addEventListener('click', function () {
	...
	var color_polygone = $('.selected3').attr('name');
...
drawPolylineButton.addEventListener('click', function () {
	$('#toolbar').hide();
	var color_polyline = $('.selected3').attr('name');
	$('#ajout_pins').css('display', 'none');
...
$('.speed').change(function () { //lorsque le coef de vitesse change on refait les trajets desbateaux qui bougent
	...
	for (var j = 0; j <= bateaux.length - 1; j++) { //boucle de for de tous les bat
		...
		var DistanceTotaleM = 0;
			...
			bateaux[j] = new L.Marker.movingMarker(currentPolyline2._latlngs, TOTAL, editIcon).addTo(map);
...
stopEdtiPolyline.addEventListener('click', function () {
	...
	bateaux[id] = new L.Marker.movingMarker(currentPolyline2._latlngs, TOTAL, editIcon).addTo(map);
	//edit de bateaux
...
console.log(TOTAL);
		...
		//var elem = "bateau" + bat;
		bateaux[a] = L.Marker.movingMarker(currentPolyline._latlngs, TOTAL, CurrentIcon).addTo(map);
